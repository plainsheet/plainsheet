# Quick Start Guide

## Installation

### Using NPM

```shell copy
npm install @plainsheet/core
```

### Using CDN

```html copy
<script src="https://cdn.jsdelivr.net/npm/@plainsheet/core@latest/dist/core.umd.js"></script>

// Or, via unpkg
<script src="https://unpkg.com/@plainsheet/core@latest/dist/core.umd.js"></script>
```

## Getting Started

### Using NPM

```ts copy
import { createBottomSheet } from "@plainsheet/core";

const bottomSheet = createBottomSheet({
  content: `<div> 
        Your content
        </div>`,
});

bottomSheet.mount();
bottomSheet.open();

// Change the content, on the fly.
bottomSheet.props.content = `<div>New Content</div>`;
```

### Using CDN

```ts copy
const bottomSheet = window.PlainBottomSheetCore.createBottomSheet({
  content: ``,
});
// Rest is the same as NPM
```
